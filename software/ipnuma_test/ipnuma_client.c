...
//write	(1,sdata,18+(4*dwlen*2)+1);
		...
		sdata[ 6] = (write_addr >> 56) & 0xff;	// ADDR=0xD0000000
		sdata[ 7] = (write_addr >> 48) & 0xff;
		sdata[ 8] = (write_addr >> 40) & 0xff;
		sdata[ 9] = (write_addr >> 32) & 0xff;
		sdata[10] = (write_addr >> 24) & 0xff;
		sdata[11] = (write_addr >> 16) & 0xff;
		sdata[12] = (write_addr >>  8) & 0xff;
		sdata[13] = (write_addr >>  0) & 0xff;

		sdata[14] = (write_data >>  0) & 0xff;
		sdata[15] = (write_data >>  8) & 0xff;
		sdata[16] = (write_data >> 16) & 0xff;
		sdata[17] = (write_data >> 24) & 0xff;
		sdata[18] = 0x00; // delimiter

		if (sendto(sockfd, sdata, 14+4+1, 0, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0){
			perror("sendto()");
		}
