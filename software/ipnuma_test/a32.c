...
//write	(1,sdata,18+(4*dwlen*2)+1);
		...
		sdata[ 6] = (write_addr >> 24) & 0xff;
		sdata[ 7] = (write_addr >> 16) & 0xff;
		sdata[ 8] = (write_addr >>  8) & 0xff;
...
while (1) {
		...
		if (sendto(sockfd, sdata, 10+4+1, 0, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0){
			perror("sendto()");
		}
		usleep(1);
}
